...
# 한글 폰트 설정
mpl.rcParams['font.family'] = 'NanumGothic'  # 사용 가능한 한글 폰트로 변경
...
if uploaded_file is not None:
    ...
    if selected_rows and selected_columns:
        ...
        ax.set_yticks(indices + (bar_width + gap) * len(selected_columns) / 2, selected_rows, fontsize=12)  # 글자 크기를 12로 조정

        # 범례 설정
        ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.15), ncol=min(3, total_bars))  # 범례 위치 및 열 개수 조정
